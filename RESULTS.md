...
- **✅ النشر الفعلي تم على جهازي حافة حقيقيين، للأوضاع الثلاثة كاملة**: Jetson
  Nano (ONNX Runtime CPU 10.1-21.4 FPS، و**TensorRT+GPU 95.8-176.7 FPS،
  تسريع 8.3×-9.5×**) وRaspberry Pi 4 (ONNX Runtime CPU **15.0-37.7 FPS** -
  أسرع من Jetson Nano على مسار CPU البحت) - راجع القسم 6.
...
**تحديث 2026-08-02:** النشر الفعلي تحقَّق بالكامل عملياً على **جهازي حافة
حقيقيين** (Jetson Nano وRaspberry Pi 4)، شامل الأوضاع الثلاثة على كليهما،
واختبار الكاميرا الحي، وإثبات DIC تحت حمل حقيقي (الأقسام 6 و6-ب) - مسار
ONNX Runtime (CPU، على الجهازين) ومسار TensorRT (GPU، Jetson Nano فقط)
كلاهما يعملان فعلياً ومُقاسان بأرقام حقيقية. **لا يوجد أي بند متبقٍ ضروري
لاكتمال الهدف الأساسي للمشروع.** كل ما تبقى (ablation، خطوط الأساس،
L2Norm+C1) تحسينات علمية إضافية موثَّقة في `NEXT_STEPS.md`، مش شروطاً
...
**لم يُنفَّذ بعد (يحتاج GPU و/أو وقتاً أطول):** جدول ablation حقيقي (4
صفوف)، خطوط الأساس (`ssd300_vgg16`، `ssdlite320_mobilenet_v3_large`)،
تعديلات المعمارية (L2Norm+C1، switchable BN)، إعادة نشر checkpoint الجولة
الثالثة على Jetson Nano (TensorRT)، اختبار DIC الفعلي تحت حمل على الجهاز،
وقياس الطاقة. التفاصيل والأوامر الجاهزة للتشغيل موثَّقة في `NEXT_STEPS.md`.
